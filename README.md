...
Dieses Repository enthält statische Seiten sowie einen serverseitigen Handler `api/contact.php` für den Versand über den IONOS-SMTP-Server. Die folgenden Hinweise beantworten die häufigsten Fragen aus dem Betrieb.
...
Die Werte müssen mit deinem Postfach bei IONOS übereinstimmen. `IONOS_FROM` sollte exakt dem Absender entsprechen, den IONOS zulässt (meist die Postfachadresse). Bewahre die `.env` Datei nur lokal oder in Secret-Stores deiner Hosting-Plattform auf. Das PHP-Backend liest die `.env` aus dem Repo-Root, falls keine Umgebungsvariablen gesetzt sind.

## Hosting & Sicherheit
* **Serverloses Backend erforderlich:** GitHub Pages liefert nur statische Dateien. Der Fetch-Aufruf des Kontaktformulars (`/api/contact.php`) braucht deshalb ein Hosting-Anbieter, der serverseitige Verarbeitung unterstützt (z. B. PHP auf klassischen Webspace oder Node.js-Funktionen mit geheimen Umgebungsvariablen wie Vercel/Netlify).
